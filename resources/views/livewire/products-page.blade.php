          ...
          <div class="w-full pr-2 lg:w-1/4 lg:block">
            ...
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
              ...
              <ul>
                ...
                <li class="mb-4" wire:key="{{$brands->id}}">
                  <label for="{{$brands->slug}}" class="flex items-center dark:text-gray-300">
                    <input type="checkbox" wire:model.live="selected_brands" class="w-4 h-4 mr-2" id="{{$brands->slug}}"
                    value="{{$brands->id}}">
                    <span class="text-lg dark:text-gray-400">{{$brands->name}}</span>
                  </label>
                </li>
                @endforeach
              </ul>
            ...
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
              ...
              <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
              <div>
                <div class="font-semibold">IDR. 0 - {{Number::currency($price_range, 'IDR')}}</div>
                <input type="range" wire:model.live="price_range" class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer" max="10000000" value="10000000" step="300000">
                <div class="flex justify-between ">
                  <span class="inline-block text-md font-bold text-blue-400 ">{{Number::currency(150000, 'IDR')}}</span>
                  <span class="inline-block text-md font-bold text-blue-400 ">{{Number::currency(10000000, 'IDR',)}}</span>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div class="border border-gray-300 dark:border-gray-700">
                      <div class="relative bg-gray-200">
                        <a href="/products/{{$prod->slug}}" class="">
                          <img src="{{ url('storage', $prod->image[0])}}" alt="{{$prod->name}}" class="object-cover hover:object-contain w-full h-56 mx-auto ">
                        </a>
                      </div>
                      <div class="p-3 ">
                        ...
                        <p class="text-lg ">
                          <span class="text-green-600 dark:text-green-600">{{Number::currency($prod->price, 'IDR')}}</span>
                        </p>
                      </div>
                      <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">

                        <a href="#" wire:click.prevent="addToCart({{$prod->id}})" class="text-gray-500 flex items-center space-x-2 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
                          ...
                          </svg><span wire:loading.remove wire:target="addToCart({{ $prod->id }})">Add to Cart</span>
